...
_req_counter = 0
_req_counter_lock = asyncio.Lock()
_req_num = 0
...
class TestApp(BaseAppAuth):
    @staticmethod
    async def statistic(message: RequestMessage, req_name: str, res: dict = None):
        global _req_item, _req_counter
        if ArgParse.getter('detail'):
            message.log_write(res)
        _req_item[req_name] += 1
        async with _req_counter_lock:
            _req_counter += 1
    ...
    async def get_stat_slice(self):
        global _req_counter, _req_num
        _logger_print = MainLogger.main_logger().info
        _counter = 0
        delay = 10
        while _req_counter < _req_num and _counter < delay * 60:
            _counter += 1
            await asyncio.sleep(1 / delay)
        if _counter >= delay * 60:
            MainLogger.main_logger().warning(f"Counter fail: {_req_counter}/{_req_num}")
        else:
            _logger_print(f"Counter: {_req_counter}/{_req_num}")
    ...
    async def get_item(self, item_id: int, req: Request):
        ...
        await self.statistic(message, 'get_item', res)
        return res
    ...
    async def new_item(self, req: Request):
        message = await RequestMessage().get_from_request(req)
        res = {"success": True}
        await self.statistic(message, 'new_item', res)
        return res
    ...
    async def update_item(self, item_id: int, req: Request):
        message = await RequestMessage().get_from_request(req)
        res = {"success": True, "item_id": item_id} if item_id else {"success": False}
        await self.statistic(message, 'update_item', res)
        return res
